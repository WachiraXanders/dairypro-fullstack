    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-lg overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-5 mt-6">
          ...
          <div className="space-y-2">
            ...
            {selectedItem && (
              <div className="flex items-center justify-between text-xs text-slate-500 bg-slate-50 px-3 py-2 rounded-lg">
                <span>Available: <strong className="text-slate-700">{selectedItem.total_quantity_kg} kg</strong></span>
                {selectedItem.cost_per_kg && (
                  <span>Cost: <strong className="text-slate-700">Kshs {selectedItem.cost_per_kg}/kg</strong></span>
                )}
              </div>
            )}
          </div>
          ...
          {/* Calculated preview */}
          {qty > 0 && selectedItem && (
            <div className={`p-3 rounded-lg border space-y-1.5 ${willTriggerAlert ? 'bg-amber-50 border-amber-200' : 'bg-slate-50 border-slate-200'}`}>
              {headCount > 0 && (
                <div className="flex justify-between text-sm">
                  <span className="text-slate-500">Kg per head</span>
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className={`font-semibold ${stockAfter < 0 ? 'text-rose-600' : willTriggerAlert ? 'text-amber-600' : 'text-slate-700'}`}>
