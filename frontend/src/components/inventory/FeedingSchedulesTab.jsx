  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="font-semibold text-slate-800">Automatic Feeding Schedules</h3>
          <p className="text-sm text-slate-500">Feed entries and stock deductions are created automatically each day this schedule is active.</p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
        {isLoading ? (
          ...
          <p className="text-center py-10 text-slate-400 text-sm">No feeding schedules yet.</p>
        ) : (
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {schedules.map((s) => (
                <TableRow key={s.id}>
                  ...
                  <TableCell className="text-xs text-slate-500">{s.start_date} → {s.end_date}</TableCell>
                  <TableCell>
      ...
      <div>
        <h3 className="font-semibold text-slate-800 mb-2 flex items-center gap-2"><CalendarClock className="w-4 h-4" /> Recent Auto-Generated Feed Entries</h3>
        <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
          {feedRatios.length === 0 ? (
            <p className="text-center py-10 text-slate-400 text-sm">No feed entries logged yet.</p>
          ) : (
            <Table>
              <TableHeader>
