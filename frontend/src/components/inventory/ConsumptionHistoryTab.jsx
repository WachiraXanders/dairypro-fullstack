    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-white border border-slate-100 rounded-xl p-4">
          <p className="text-xs text-slate-500 mb-1">This Month – Total Consumed</p>
          <p className="text-xl font-bold text-slate-800">{stats.totalKg.toFixed(1)} kg</p>
        </div>
        <div className="bg-white border border-slate-100 rounded-xl p-4">
          <p className="text-xs text-slate-500 mb-1">This Month – Feed Cost</p>
        ...
        <div className="bg-white border border-slate-100 rounded-xl p-4">
          ...
          <p className="text-xl font-bold text-slate-800">{stats.entries}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
        {isLoading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-slate-500 font-medium">No consumption records yet</p>
            <p className="text-xs text-slate-400 mt-1">Use "Log Consumption" to record feed usage</p>
          </div>
        ) : (
          <Table>
            <TableHeader>
                ...
                <TableRow key={r.id} className="hover:bg-slate-50">
                  ...
                  <TableCell>
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-sm font-medium text-slate-800">{r.item_name}</span>
                    </div>
                  </TableCell>
                  <TableCell>
                    {r.group_name ? (
                  ...
                  <TableCell>
                    <span className="font-semibold text-slate-800">{r.quantity_kg}</span> kg
                  </TableCell>
                  ...
                  <TableCell>
                    {r.stock_after_kg !== undefined && r.stock_after_kg !== null ? (
                      <span className={cn(
                        "text-sm font-medium",
                        r.stock_after_kg <= 0 ? 'text-rose-600' : r.stock_after_kg < 50 ? 'text-amber-600' : 'text-slate-600'
                      )}>
                        {r.stock_after_kg} kg
                      </span>
                    ) : '—'}
