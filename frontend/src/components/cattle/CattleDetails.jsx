  ...
  const InfoRow = ({ icon: Icon, label, value }) => (
    <div className="flex items-start gap-3 p-3 rounded-lg bg-slate-50">
      ...
      <div>
        ...
        <p className="text-sm font-medium text-slate-800">{value || 'Not specified'}</p>
      </div>
    </div>
  );
        ...
        <div className="mt-6 space-y-6">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-slate-500 mt-1">Tag: {cattle.tag_number}</p>
          ...
          <Tabs defaultValue="overview" className="w-full">
            ...
            <TabsContent value="overview" className="space-y-4 mt-4">
              ...
              {(cattle.sire_id || cattle.dam_id) && (
                <div className="p-4 rounded-xl bg-slate-50">
                  <h3 className="text-sm font-semibold text-slate-700 mb-3">Lineage</h3>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <p className="text-xs text-slate-500">Sire (Father)</p>
                      <p className="text-sm font-medium text-slate-800">{cattle.sire_id || 'Unknown'}</p>
                    </div>
                    <div>
                      <p className="text-xs text-slate-500">Dam (Mother)</p>
                      <p className="text-sm font-medium text-slate-800">{cattle.dam_id || 'Unknown'}</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              {cattle.notes && (
                <div className="p-4 rounded-xl bg-slate-50">
                  ...
                  <p className="text-sm text-slate-600 whitespace-pre-wrap">{cattle.notes}</p>
                </div>
              )}
            </TabsContent>
            ...
            <TabsContent value="breeding" className="space-y-3 mt-4">
              {breedingRecords.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-slate-500 mt-2">No breeding records</p>
                </div>
              ) : (
                <>
                  {lastCalving && (
                  ...
                  <div className="space-y-2">
                    <h4 className="text-sm font-semibold text-slate-700">Breeding History</h4>
            ...
            <TabsContent value="health" className="space-y-3 mt-4">
              {healthRecords.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-slate-500 mt-2">No health records</p>
                </div>
              ) : (
                <div className="space-y-2">
                  {healthRecords.map((record) => (
                    <div key={record.id} className="p-3 rounded-lg bg-slate-50 border border-slate-200">
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <p className="text-sm font-medium text-slate-800">{record.record_type}</p>
                          <p className="text-xs text-slate-600 mt-0.5">
                            {format(new Date(record.date), 'MMM d, yyyy')}
                          </p>
                          {record.diagnosis && (
                            <p className="text-xs text-slate-700 mt-1">{record.diagnosis}</p>
                          )}
                          {record.treatment && (
                            <p className="text-xs text-slate-600 mt-0.5">Treatment: {record.treatment}</p>
                          )}
                        </div>
                        <Badge className={cn("text-xs",
            ...
            <TabsContent value="production" className="space-y-3 mt-4">
              {milkProduction.length === 0 ? (
                <div className="text-center py-8">
                  <Milk className="w-8 h-8 mx-auto text-slate-300" />
                  <p className="text-sm text-slate-500 mt-2">No production records</p>
                </div>
              ) : (
                <>
                  <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-2">
                    <h4 className="text-sm font-semibold text-slate-700">Recent Production</h4>
                    {milkProduction.slice(0, 7).map((record) => (
                      <div key={record.id} className="p-3 rounded-lg bg-slate-50 border border-slate-200 flex items-center justify-between">
                        <div>
                          <p className="text-sm font-medium text-slate-800">
                            {format(new Date(record.date), 'MMM d')}
                          </p>
                          <p className="text-xs text-slate-600">{record.session}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-lg font-bold text-emerald-700">{record.quantity_liters}L</p>
                          {record.quality_grade && (
                            <p className="text-xs text-slate-500">Grade {record.quality_grade}</p>
                          )}
                        </div>
                      </div>
                    ))}
