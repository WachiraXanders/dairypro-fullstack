...
function Trend({ value, suffix = '', invert = false }) {
  ...
  if (neutral) return <span className="flex items-center gap-1 text-slate-400 text-xs"><Minus className="w-3 h-3" /> —</span>;
  return (
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          <CardContent className="pt-5">
            <p className="text-xs text-slate-500">Cattle Analysed</p>
            <p className="text-2xl font-bold text-slate-800">{kpiData.length}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-5">
            <p className="text-xs text-slate-500">Avg Milk-to-Feed Ratio</p>
            <p className="text-2xl font-bold text-emerald-600">{avgMilkToFeed.toFixed(2)} L/kg</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-5">
            <p className="text-xs text-slate-500 flex items-center gap-1"><Award className="w-3 h-3 text-yellow-500" />Top Performers</p>
            <p className="text-2xl font-bold text-yellow-600">{topPerformers.length}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-5">
            <p className="text-xs text-slate-500 flex items-center gap-1"><AlertTriangle className="w-3 h-3 text-rose-500" />Needs Attention</p>
      ...
      <Card>
        ...
        <CardContent className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {kpiData.map((cow, i) => (
                <tr key={cow.id} className="border-b last:border-0 hover:bg-slate-50">
                  <td className="py-2.5 pr-4 font-medium text-slate-400">#{i + 1}</td>
                  <td className="py-2.5 pr-4">
                    <div className="font-medium text-slate-800">{cow.name}</div>
                    <div className="text-xs text-slate-400">Tag: {cow.tag}</div>
                  </td>
                  <td className="py-2.5 pr-4 text-slate-500">{cow.breed || '—'}</td>
                  ...
                  <td className="py-2.5 pr-4 text-right text-slate-600">
                    {cow.feedCostPerLiter !== null ? `Kshs ${cow.feedCostPerLiter}` : <span className="text-slate-300">—</span>}
                  </td>
                  <td className="py-2.5 pr-4 text-right text-slate-600">
                    {cow.adg !== null ? cow.adg : <span className="text-slate-300">—</span>}
                  </td>
                  <td className="py-2.5 text-right font-medium text-blue-600">{cow.totalMilkLifetime}</td>
                </tr>
              ))}
              {kpiData.length === 0 && (
                <tr><td colSpan={9} className="py-8 text-center text-slate-400">No active cattle found</td></tr>
              )}
            </tbody>
          </table>
        </CardContent>
