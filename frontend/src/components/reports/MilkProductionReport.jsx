    ...
    <div className="space-y-6">
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div>
            <h3 className="font-semibold text-slate-800 mb-3">Total Milk Since First Record (All-Time)</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {Object.values(summary.lifetimeByCattle).sort((a, b) => b.total - a.total).map((cow, idx) => (
                <div key={idx} className="bg-teal-50 border border-teal-100 rounded-lg p-3">
                  <p className="text-sm font-medium text-slate-700">{cow.tag} {cow.name ? `- ${cow.name}` : ''}</p>
                  <p className="text-xl font-bold text-teal-700">{cow.total.toFixed(1)} L</p>
                  <p className="text-xs text-slate-500">Since {cow.firstMilkDate ? format(new Date(cow.firstMilkDate), 'MMM d, yyyy') : '—'}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* By Cattle (period) */}
          <div>
            <h3 className="font-semibold text-slate-800 mb-3">Production by Cattle (Selected Period)</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {Object.values(summary.byCattle).map((cow, idx) => (
                <div key={idx} className="bg-white border rounded-lg p-3">
                  ...
                  <p className="text-xs text-slate-500">{cow.count} records</p>
                </div>
              ))}
            </div>
          </div>
