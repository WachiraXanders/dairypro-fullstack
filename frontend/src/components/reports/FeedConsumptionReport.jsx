    ...
    <div className="space-y-6">
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          {/* By Feed Type */}
          <div>
            <h3 className="font-semibold text-slate-800 mb-3">Consumption by Feed Type</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {Object.values(summary.byFeed).map((feed, idx) => (
                <div key={idx} className="bg-white border rounded-lg p-3">
                  <p className="text-sm font-medium text-slate-700">{feed.name}</p>
                  <p className="text-lg font-bold text-emerald-600">{feed.totalKg.toFixed(1)} kg</p>
                  <p className="text-xs text-slate-500">Kshs {feed.totalCost.toFixed(2)}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* By Cattle */}
          <div>
            <h3 className="font-semibold text-slate-800 mb-3">Consumption by Cattle</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {Object.values(summary.byCattle).map((cow, idx) => (
                <div key={idx} className="bg-white border rounded-lg p-3">
                  <p className="text-sm text-slate-600">{cow.tag} - {cow.name}</p>
                  <p className="text-lg font-bold text-emerald-600">{cow.totalFeed.toFixed(1)} kg</p>
                  <p className="text-xs text-slate-500">Kshs {cow.totalCost.toFixed(2)} • {cow.count} entries</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Detailed Table */}
          <div>
            <h3 className="font-semibold text-slate-800 mb-3">Detailed Records ({filteredData.length})</h3>
                ...
                <TableBody>
                  {filteredData.map((record) => (
                    <TableRow key={record.id}>
                      ...
                      <TableCell className="text-slate-500">{record.remaining_inventory_kg || 0} kg</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
