    ...
    <div className="space-y-6">
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          {/* By Category */}
          <div>
            <h3 className="font-semibold text-slate-800 mb-3">Inventory by Category</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {Object.entries(summary.byCategory).map(([category, data]) => (
                <div key={category} className="bg-white border rounded-lg p-3">
                  <p className="text-sm font-medium text-slate-700">{category}</p>
                  <p className="text-lg font-bold text-emerald-600">{data.count} items</p>
                  <p className="text-xs text-slate-500">{data.totalKg.toFixed(1)} kg • Kshs {data.value.toFixed(2)}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Detailed Table */}
          <div>
            <h3 className="font-semibold text-slate-800 mb-3">Inventory Details ({filteredData.length})</h3>
