      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <Card className="lg:col-span-2">
          <CardHeader className="pb-3">
            ...
            <div className="flex gap-3 pt-2 text-xs text-slate-500">
          ...
          <CardContent>
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      entries.length > 0 ? intensityClass(entries.length) : (today ? "border-emerald-400" : "border-transparent hover:border-slate-200 hover:bg-slate-50"),
            ...
            {selectedDay && (
              <div className="mt-4 p-3 bg-slate-50 rounded-lg border">
                <p className="text-sm font-semibold text-slate-700 mb-2">{format(selectedDay, 'EEEE, MMMM d, yyyy')}</p>
                {selectedEntries.length === 0 ? (
                  <p className="text-sm text-slate-400">No calvings</p>
                ) : (
                  <div className="space-y-1.5">
                    {selectedEntries.map((e, i) => (
                      <div key={i} className="flex items-center justify-between px-2 py-1.5 rounded-md bg-white border text-sm">
        ...
        <Card>
          ...
          <CardContent>
            {upcoming.length === 0 ? (
              <p className="text-sm text-slate-400 text-center py-6">No expected calvings in the next 60 days</p>
            ) : (
                ...
                {upcoming.map((e, i) => (
                  <div key={i} className="flex items-start gap-2 p-2 rounded-lg bg-emerald-50">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-slate-600 truncate">{e.tag} {e.name && `— ${e.name}`}</p>
                    </div>
                  </div>
                ))}
              </div>
